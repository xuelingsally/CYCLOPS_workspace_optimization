...
FException::FException(char *error_msg, ...) {
	...
	printf(this->error_msg, (size_t)FE_MESSAGE_BUFFER_SIZE, error_msg, arg_list);
	va_end(arg_list);
 ...
 * @returns FException
 */
FException::FException(unsigned int error_code, char *error_msg, ...) {
	va_list arg_list;
 ...
 */
unsigned int FException::getErrorCode() {
