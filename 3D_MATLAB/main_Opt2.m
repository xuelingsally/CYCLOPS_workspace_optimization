...
eaB_max(1:6) = eaB_max(1:6) * 220/180 * pi;
%Limits for a
eaB_min(7:12) = eaB_min(7:12) * (-dist_tool_b_cg);
eaB_max(7:12) = eaB_max(7:12) * (length_overtube - dist_tool_b_cg);
%Limits for b
eaB_min(13:18) = eaB_min(13:18) * (-length_scaffold);
eaB_max(13:18) = eaB_max(13:18) * 0;
...
nvars = 18;
fun = @(x) -my_objective_function(x, W, f_ee, r_ee, phi_min, phi_max, t_min, t_max, taskspace_d/1000, radius_tool, radius_scaffold);
[eaB,fval,exitflag] = particleswarm(fun,nvars,eaB_min,eaB_max);

%% Genetic Algorithm
% nvars = 18;
% fun = @(x) -my_objective_function(x, W, f_ee, r_ee, phi_min, phi_max, t_min, t_max, taskspace_d/1000, radius_tool, radius_scaffold);
% [eaB,fval,exitflag] = ga(fun,nvars,[],[],[],[],eaB_min,eaB_max);
