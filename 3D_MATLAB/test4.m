...
% r_ee_dir is the vector from the curving point to the end effector of the tool
curve_length2 = norm(r_ee_dir2);


r_ee = r_ee_dir2 + r_ee_dir + r_curve;
...
for i=1:size(data1,2)
    ...
    temp = R * [curve_length2;0;0];
    
    data_t1(1:3,i) = -temp + data1(1:3,i);
    
    
    
    beta_y = alpha_y - gamma2_y;
    beta_z = alpha_z - gamma2_z;
    
    R_y_2 = [cos(beta_y), 0, sin(beta_y); 0, 1, 0; -sin(beta_y), 0, cos(beta_y)];
    R_z_2 = [cos(beta_z), -sin(beta_z), 0; sin(beta_z), cos(beta_z), 0; 0, 0, 1];
    R_2 = R_z_2 * R_y_2;
    
    temp2 = R_2 * [curve_length;0;0];
    data_t2(1:3,i) = -temp2 + data_t1(1:3,i);
    
    xi_y = beta_y - gamma_y;
    xi_z = beta_z - gamma_z;
    
    R_y_3 = [cos(xi_y), 0, sin(xi_y); 0, 1, 0; -sin(xi_y), 0, cos(xi_y)];
    R_z_3 = [cos(xi_z), -sin(xi_z), 0; sin(xi_z), cos(xi_z), 0; 0, 0, 1];
    R_3 = R_z_3 * R_y_3;
