...
% if abs(eaB(13) - eaB(14)) < abs(min(eaB(7:9)) - max(eaB(10:12)))
%     val = -1000;
%     return;
% end
...
for i=1:size(taskspace_d,2)
    taskspace(:,i) = (taskspace_d(:,i) - [dist_tooltip; 0; 0])/1000;
end
...
if val < 0
    val = val/(size(taskspace,2) * size(f_ee,1));
    return
end
